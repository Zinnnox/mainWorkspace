...
public class IceshardRenderer extends EntityRenderer<IceshardEntity> {
	private static final ResourceLocation texture = ResourceLocation.parse("cristian_s_master:textures/entities/iceshard.png");
	private final Modeliceshard model;

	public IceshardRenderer(EntityRendererProvider.Context context) {
		super(context);
		model = new Modeliceshard(context.bakeLayer(Modeliceshard.LAYER_LOCATION));
	}

	@Override
	public void render(IceshardEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
