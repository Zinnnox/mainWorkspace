...
import net.mcreator.cristiansmaster.procedures.ShootFireballCheckProcedure;
...
public record ShootFireballMessage(int eventType, int pressedms) implements CustomPacketPayload {
	...
	}, (RegistryFriendlyByteBuf buffer) -> new ShootFireballMessage(buffer.readInt(), buffer.readInt()));

	@Override
	public Type<ShootFireballMessage> type() {
		return TYPE;
	}

	public static void handleData(final ShootFireballMessage message, final IPayloadContext context) {
	...
	public static void pressAction(Player entity, int type, int pressedms) {
		...
		if (type == 0) {
			...
			ShootFireballCheckProcedure.execute(world, x, y, z, entity);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		CristianSMasterMod.addNetworkMessage(ShootFireballMessage.TYPE, ShootFireballMessage.STREAM_CODEC, ShootFireballMessage::handleData);
	}
}
